...
class Karma(object):
    ...
    def __init__(self, min_ver_support, epsilon, num_relations=7, max_gap=50, label=0, selected_variables=[]):
    ...
    def fit(self, file_path, num_comma=3, entity_ids_num=1, skip_followers=False, symbol_type='int', index_same=False,
            semicolon_end=False, calc_offsets=False, filter_overlapping_grad_state=False):
        """
        ...
        self.build_karma_index(skip_followers, index_same, filter_overlapping_grad_state)
        self._logger.debug('done building karma index')
        self._symbols_map = OrderedDict(sorted(self._symbols_map.items(), key=lambda t: t[0]))
        self._logger.debug('done sorting karma symbols')
    ...
    def parse_file_into_symbolic_time_intervals_parts(self, file_path, start_index, symbol_type, num_comma,
        ...
        for i in range(0, len(lines), 2):
            ...
            for sym_ti in intervals_list:
                sym_ti_obj, symbol = parser_of_sym_ti_row_for_different_formats(sym_ti, num_comma, symbol_type)
    ...
    def build_karma_index(self, skip_followers, index_same, filter_overlapping_grad_state):
        """
            ...
            for ti1_index in range(len(tis_list)):
                ...
                for ti2_index in range(ti1_index + 1, len(tis_list)):
                    ...
                    if skip_followers and abs(float(first_tis._symbol) - float(second_tis._symbol)) == 1 and \
                        ...
                        continue
                    if filter_overlapping_grad_state and first_tis._var_id is not None \
                            and first_tis._var_id == second_tis._var_id and \
                            first_tis._end_time >= second_tis._start_time:
                        continue
